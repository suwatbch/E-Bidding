...
export default function Dropdown({ isOpen, onClose, trigger, children, variant = 'navbar' }: DropdownProps) {
  ...
  const styles = {
    navbar: {
      container: "relative",
      dropdown: "absolute right-0 mt-2 w-48 bg-white/95 backdrop-blur-xl rounded-xl shadow-lg py-1.5 border border-white/20",
      item: "flex items-center w-full px-4 py-2.5 text-sm text-gray-700 hover:bg-blue-50/50 transition-all duration-300"
    },
    mobile: {
      container: "w-full",
      dropdown: "w-full mt-1 bg-white/95 backdrop-blur-xl rounded-xl shadow-lg py-1.5 border border-white/20",
