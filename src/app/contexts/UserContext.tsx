...
export function UserProvider({ children }: { children: React.ReactNode }) {
  ...
  const syncWithInitialData = (userData: User): User => {
    const initialUser = initialUsers.find(u => u.user_id === userData.user_id);
    if (initialUser) {
      ...
      return {
        ...
        language_code: userData.language_code || initialUser.language_code || 'th',
    ...
    try {
      ...
      if (storedUsers) {
        ...
        if (Array.isArray(parsedUsers) && parsedUsers.length > 0) {
          ...
          const updatedUsers = parsedUsers.map(user => syncWithInitialData(user));
